(function () {
    ...
    function renderBrands() {
        ...
        var rows = state.brands.filter(function (brand) {
            var matchedKeyword =
                !keyword ||
                String(brand.name || '').toLowerCase().indexOf(keyword) !== -1;
    ...
    function renderModels() {
        ...
        var rows = state.models.filter(function (model) {
            return (!brandId || String(model.brand_id) === brandId)
    ...
    function renderVersions() {
        ...
        var rows = state.versions.filter(function (version) {
            return (!modelId || String(version.model_id) === modelId)
        ...
        tbody.innerHTML = rows.map(function (version) {
            var years = [
                version.production_year_from,
                version.production_year_to
            ].filter(Boolean).join(' - ');
    ...
    function modelLabel(model) {
        return (model.brand_name ? model.brand_name + ' - ' : '')
            + model.name
    ...
    function refreshVersionModelSelect(selectedModelId) {
        ...
        var filteredModels = state.models.filter(function (model) {
            return String(model.brand_id) === String(brandId)
                && model.status === 'active';
        });

        modelSelect.innerHTML =
            '<option value="">-- Chọn dòng xe --</option>'
            + options(filteredModels, 'id', function (model) {
                return model.name;
            }, false);
    ...
    function save() {
        ...
        request(method, url, payload)
            .then(function () {
                notify(
                    id ? 'Cập nhật thành công.' : 'Thêm mới thành công.',
                    'success'
                );
    ...
    function removeOne(kind, id) {
        ...
        var url = kind === 'brand'
            ? API.brand(id)
            : (kind === 'model' ? API.model(id) : API.version(id));
    ...
    function switchSection(section) {
        ...
        ['brands', 'models', 'versions'].forEach(function (name) {
            ...
            var nav = document.querySelector('.admin-nav-item[data-section="' + name + '"]');
            if (nav) {
