(function ($) {
  ...
  function openCropper(file) {
    ...
    lastChosenFileName =
      (file.name || "brand-logo").replace(/\.[^.]+$/, "") + ".webp";
    ...
    cropImage.onload = function () {
      try {
        ...
        showToast("Không thể khởi tạo công cụ căn chỉnh ảnh.", "error");
      }
    };
  ...
  function uploadCroppedLogo() {
    ...
    try {
      ...
      console.error("getCroppedCanvas error:", e);

      showToast("Không thể xử lý ảnh đã cắt.", "error");
    ...
    canvas.toBlob(
      function (blob) {
        ...
        console.log("Cropped blob:", blob.size, blob.type);
        ...
        $.ajax({
          ...
          .done(function (response) {
            ...
            var upload =
              response && response.data && response.data.upload
            ...
            setUploadedLogo(upload.secure_url, upload.public_id || "");
  ...
  $(document).on("click", "#brandLogoOpen", function (event) {
  ...
  $(document).on("change", "#brandLogoFile", function () {
